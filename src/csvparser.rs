...
impl std::fmt::Display for ParsingState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ParsingState::None => write!(f, "ParsingState::None"),
            ParsingState::InterestsPLN(_) => write!(f, "ParsingState::InterestsPLN"),
            ParsingState::Crypto(_) => write!(f, "ParsingState::Crypto"),
            ParsingState::InterestsEUR(_) => write!(f, "ParsingState::InterestsEUR"),
            ParsingState::SellUSD(_) => write!(f, "ParsingState::SellUSD"),
            ParsingState::SellEUR(_) => write!(f, "ParsingState::SellEUR"),
...
fn extract_cash(cashline: &str) -> Result<crate::Currency, String> {
    ...
    } else if cashline.contains(',') {
        // If there is only comma we need to check if this is separation of thousends or actual
        // decimal separator
        let after_comma = cashline.split(',').last().unwrap_or("");
        let digits_after_comma: String =
            after_comma.chars().filter(|c| c.is_ascii_digit()).collect();

        if digits_after_comma.len() == 3 {
            cashline.to_string().replace(",", "") // Separator of thousends (e.g. 1,000.00$)
        } else {
            cashline.to_string().replace(",", ".") // Separator of decimal (eg. 63,28$)
        }
    } else {
        cashline.to_string()
    };
    ...
    let mut euro_parser3 = tuple((many_m_n(0, 1, tag("-")), tag("€"), double::<&str, Error<_>>));
    ...
    if let Ok((_, (value, _))) = euro_parser(cashline_string.as_str()) {
        ...
        return Ok(crate::Currency::EUR(value));
    } else if let Ok((_, (_, _, value))) = euro_parser3(cashline_string.as_str()) {
        return Ok(crate::Currency::EUR(value));
    ...
    } else if let Ok((_, (_, _, value))) = usd_parser4(cashline_string.as_str()) {
        return Ok(crate::Currency::USD(value));
    } else {
...
fn extract_sold_transactions(df: &DataFrame) -> Result<DataFrame, &'static str> {
            ...
            .any(|col| col.contains("Data") && col.contains("Sprzedaży"))
    {
        // English or Polish column names for v2 format
        // Note: Revolut uses non-breaking spaces (U+00A0) in newer Polish headers
        log::info!("Sold transactions before renaming: {df}");
        let date_col = df
            .get_column_names()
            .iter()
            .find(|&col| col.contains("Data (Sprzedaży)"))
            .copied()
            .unwrap_or("Date (of Sale)");

        log::info!("date_col: {date_col}");
        let purchase_date_col = df
            .get_column_names()
            .iter()
            .find(|&col| col.contains("Data (Zakupu)"))
            .copied()
            .unwrap_or("Date (of Purchase)");
        log::info!("purchase_date_col: {purchase_date_col}");
        ...
        let value_col = df
            ...
            .copied()
            .unwrap_or("Value (of Sale)");

        let cost_col = df
            .get_column_names()
            .iter()
            .find(|&col| col.contains("Wartość") && col.contains("Zakupu"))
            .copied()
            .unwrap_or("Value (of Purchase)");
    ...
    {
        df_transactions = df_transactions
            .rename(&date_col, "Date (of Sale)")
            .expect("Unable to rename Data (of Sale)")
            .clone();
    }

    if let Some(purchase_date_col) = df_transactions
        .get_column_names()
        .iter()
        .find(|&col| col.contains("Data") && col.contains("Zakupu"))
        .map(|s| s.to_string())
    {
        df_transactions = df_transactions
            .rename(&purchase_date_col, "Date (of Purchase)")
    ...
    {
        df_transactions = df_transactions
            .rename(&value_col, "Value (of Sale)")
            .expect("Unable to rename Wartość (Sprzedaży)")
            .clone();
    }
    if let Some(cost_col) = df_transactions
        .get_column_names()
        .iter()
        .find(|&col| col.contains("Wartość") && col.contains("Zakupu"))
        .map(|s| s.to_string())
    {
        df_transactions = df_transactions
            .rename(&cost_col, "Value (of Purchase)")
            .expect("Unable to rename Wartość (Zakupu)")
            .clone();
    }
...
fn extract_intrest_rate_transactions(df: &DataFrame) -> Result<DataFrame, &'static str> {
    ...
    // cols: "Completed Date", "Description" , "Money In" or "Gross interest"
    let new_desc = Series::new("Description", intrest_rate);
    let df_transactions_ref = df_transactions
        .with_column(new_desc)
        .expect("Unable to replace Description column");
    let intrest_rate_mask = df_transactions_ref
        .column("Description")
        .map_err(|_| "Error: Unable to get Description")?
        .equal("odsetki")
        .expect("Error creating mask");
...
pub fn parse_revolut_transactions(csvtoparse: &str) -> Result<RevolutTransactions, String> {
    ...
    if result.iter().any(|field| field == "Completed Date") {
        ...
        ta.stock.countries = parse_symbols(&filtred_df, "Country")?;
        ...
        log::info!("Filtered Dividend Data of interest: {filtred_df}");
        ta.dates = parse_investment_transaction_dates(&filtred_df, "Date")?;
        // parse symbols of companies and countries they are from
        ta.symbols = parse_symbols(&filtred_df, "Symbol")?;
        ta.countries = parse_symbols(&filtred_df, "Country")?;
    ...
    log::info!("Sold Symbols: {:?}", ta.stock.symbols);
    log::info!("Sold Countries: {:?}", ta.stock.countries);
    ...
    {
        return Err(format!(
            "ERROR: Different number of sold acquired_dates({}), sold_dates({}), gross({}), cost({}), symbols({}) or countries({})",
            ta.stock.acquired_dates.len(),
            ta.stock.sold_dates.len(),
            ta.stock.gross.len(),
            ta.stock.costs.len(),
            ta.stock.symbols.len(),
            ta.stock.countries.len()
        ));
    }
...
mod tests {
    ...
    fn test_extract_cash() -> Result<(), String> {
        ...
        assert_eq!(
            extract_cash("-US$20,000"),
            Ok(crate::Currency::USD(20000.0))
        );
        assert_eq!(
            extract_cash("+US$10,961.04"),
            Ok(crate::Currency::USD(10961.04))
        );
        assert_eq!(
            extract_cash("+€5,980.74"),
            Ok(crate::Currency::EUR(5980.74))
        );
    ...
    #[test]
    fn test_parse_countries() -> Result<(), &'static str> {
        let dates = vec!["25 Aug 2023", "1 Sep 2023"];
        let countries = vec!["US", "DE"];
        let expected_symbols = countries
            .iter()
            .map(|s| Some(s.to_string()))
            .collect::<Vec<Option<String>>>();

        let input_date_series = Series::new("Date", dates);
        let input_countries = Series::new("Country", countries);

        let df = DataFrame::new(vec![input_date_series, input_countries])
            .map_err(|_| "Error creating DataFrame")?;

        assert_eq!(parse_symbols(&df, "Country"), Ok(expected_symbols));
        Ok(())
    }
    ...
    fn test_parse_revolut_transactions_consolidated_crypto_tsv() -> Result<(), String> {
        ...
        parsed
            .dividend_transactions
            .iter()
            .for_each(|(_, amount, _, _, _)| match amount {
    ...
    fn test_parse_revolut_transactions_consolidated_v2_eng() -> Result<(), String> {
        ...
        assert_eq!(
            parse_revolut_transactions("revolut_data/test_v2_eng.csv"),
            expected_result
        );
        Ok(())
    }
